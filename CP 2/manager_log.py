...
class logic(Ui_MainWindow,QtWidgets.QMainWindow):
    def __init__(self):
        super(Ui_MainWindow,self).__init__()
        self.setupUi(self)
        self.setWindowTitle("Блок управления заказами")
    ...
    def get_table_inf(self):
        try:
            ...
            getTablesColumnNames = f'''SELECT column_name FROM information_schema.columns WHERE table_name = 'offer' AND table_schema = 'public' '''
            cursor.execute(getTablesColumnNames)
            self.col = cursor.fetchall()

            getTableItems = f'''SELECT * FROM offer ORDER BY offer_id'''
    ...
    def delete(self):
        try:
            ...
            connection.autocommit = True
            cursor = connection.cursor()
            DeleteRequest = f'''DELETE FROM offer where offer_id = {int(self.lineEdit_6.text())}'''
    ...
    def update(self):
        try:
            ...
            UpdateRequest = f'''UPDATE offer 
                                SET {columnName} = '{changedValue}' 
                                WHERE offer_id = {elementCode} '''
    ...
    def add(self):
        try:
            ...
            addRequest = f'''INSERT INTO offer (services_id, manager_id, instructor_id, time, fullname, phone_number)
                             VALUES ({int(columnValues[0])}, {int(columnValues[1])}, {int(columnValues[2])}, '{columnValues[3]}', '{columnValues[4]}', '{columnValues[5]}' )  '''
